...
# 工夫した点
+ スクール学習外のプラスα要素として、インフラ構築を行った
  - 本番環境をHerokuから、EC2へ変更
  - ドメインの取得
  - 常時SSL化
+ 行動につながるアプリ設計をした
...
+ 気づきを得るきっかけとなる様、様々な角度の情報を表示した
  ...
  - ☆の平均評価も表示することで、ロングスパン視点も持つことができる
...
+ テストの追加
    + モデルに定義したメソッドの異常系テストを追加
    + 非同期通信に置き換えた際、Ajaxのテストを追加
    + 繰り返す処理をモジュール化
    + ~~コントローラー単体テストを実装・実行し、品質を担保~~ 完了(計237example)
    + ~~結合テストを実装・実行し、品質を担保~~ 完了(計49example)
+ ~~解約済のサブスク編集機能を制限する~~ 完了
+ アクションプランを意識するために、実行中アクションプラン一覧表示を作成
+ 快適なブラウザ操作の実現のため、一部HTTP通信をAjaxに置き換え
+ 解約日が、更新日・契約日より前の日付の場合、確認ホップアップを表示
+ ~~エラー等を連絡するコンタクトフォームの作成~~ 完了(仮)
+ リンクの仮URLを更新
  - ~~サブスク削除ホップアップ：問い合わせリンク~~ 完了
  - サブスク編集画面：契約日を変更するリンク
+ SweetAlert2で、複数のエラーメッセージを表示する
<br>
...
+ ビューレイアウトの改善
  ...
  - 各フォームのレイアウト調整
  - ビュー全体でテイストを統一
  - ~~評価一覧に、未評価一覧へのリンクを追加~~ 完了
  - ~~マイページに、サブスク評価一覧へのリンクを追加~~ 完了
  - レビューに←→移動ボタンを追加
+ ~~ユーザー編集機能~~ 完了
+ ~~ユーザー削除機能~~ 完了
